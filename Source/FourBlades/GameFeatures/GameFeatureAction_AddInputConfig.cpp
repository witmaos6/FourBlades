...
void UGameFeatureAction_AddInputConfig::AddToWorld(const FWorldContext& WorldContext, const FGameFeatureStateChangeContext& ChangeContext)
{
	...
	if (GameInstance && World && World->IsGameWorld())
	{
		if (UGameFrameworkComponentManager* ComponentMan = UGameInstance::GetSubsystem<UGameFrameworkComponentManager>(GameInstance))
		{
			...
			// ComponentMan->AddExtensionHandler함수는 등록하는 모든 StaticClass에 바인딩을 한다. 관전자 폰이나 적 몬스터도 바인딩이 되기 때문에 문제가 될 수 있다.
			TSharedPtr<FComponentRequestHandle> ExtensionRequestHandle = ComponentMan->AddExtensionHandler(AFBCharacter::StaticClass(), AddConfigDelegate);
...
void UGameFeatureAction_AddInputConfig::HandlePawnExtension(AActor* Actor, FName EventName, FGameFeatureStateChangeContext ChangeContext)
{
	...
	if (EventName == UGameFrameworkComponentManager::NAME_ExtensionAdded || EventName == UFBCharacterComponent::NAME_BindInputsNow)
	{
